...
def load_data(csv_file, table):
    ...
    sql = "LOAD DATA LOCAL INFILE '{}' INTO TABLE {} FIELDS TERMINATED BY {} OPTIONALLY ENCLOSED BY {} " \
          "LINES TERMINATED BY {}".format(
              csv_file, table, field_termination, field_enclosing, line_termination)
    print(sql)
...
if thread_qty > 1:
    ...
    for i, reader in enumerate(readers):
        t = threading.Thread(target=write_chunk,
                             args=(fname, tmp_dir, reader, i))
    ...
    print("Finished parallel loading of {} rows in {} seconds".format(
        total_rows, total_duration))
else:
